...
### ​这是一个基于 GitHub Actions 的自动化编译脚本，专门用于为你的设备编译定制版 OrangeFox Custom Recovery。该版本已加入对16.0的全新支持，并针对最新的编译同步策略（包含特定的 sync 仓库以及震动补丁逻辑）进行了深度优化。
...
参数名称填写
 ...
 -**DEVICE_TREE_BRANCH**：你设备树的默认分支名。

 -**DEVICE_PATH**：设备树存放路径，在源码树中的绝对路径。
 
 -**DEVICE_NAME**：例如你的机器代号。
